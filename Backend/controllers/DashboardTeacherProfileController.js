...
//       return; 
//     }
//     const noOfstudents = teacher.courses.flatMap(course => course.users);
//     console.log(students);
//   });



//     res.status(200).json({ teacher, courses, noOfstudents });
//   } catch (error) {
//     console.error(error);
...
exports.dashboardTeacherProfile = async (req, res) => {
  ...
  try {
    ...
    const noOfStudents = teacher.courses.flatMap(course => course.users).length;

    res.status(200).json({ teacher, courses, noOfStudents, noofcourses });
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: 'Internal Server Error' });
  }
